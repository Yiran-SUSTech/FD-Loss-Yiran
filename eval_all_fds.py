...
def main_gen_only(args):
    ...
    # Build class schedule from --class_id_list / --num_image_per_class if given.
    ...
    if args.class_id_list:
        ...
        global_classes = np.array(
            [c for c in class_ids for _ in range(per_class)], dtype=np.int64
        )
    ...
    for ema_label in ema_labels:
        for cfg_val in cfg_list:
            ...
            if global_classes is not None:
                rank_classes = global_classes[start_idx:end_idx]
            else:
            ...
            class_records = []  # list of (global_idx, class_id) for this rank
            generated = 0
            t0 = time.perf_counter()
            with ema_model.swap(model, label=ema_label):
                while generated < local_n:
                    ...
                    imgs_np = to_uint8_numpy(images)
                    for j, img in enumerate(imgs_np):
                        gi = start_idx + generated + j
                        save_image(img, f"{eval_dir}/{gi:06d}.png")
                        if global_classes is not None:
                            class_records.append((gi, int(rank_classes[generated + j])))
                    del images, imgs_np
                    generated += b

            # Write per-rank temp class-id file; merged into class_ids.txt below.
            if global_classes is not None:
                rank_tmp = os.path.join(eval_dir, f".class_ids.rank{rank}.tmp")
                with open(rank_tmp, "w") as f:
                    for gi, cid in class_records:
                        f.write(f"{gi:06d} {cid}\n")
            ...
            # Merge per-rank temp files into class_ids.txt (rank 0 only).
            if global_classes is not None:
                if world_size > 1:
                    dist.barrier()
                if rank == 0:
                    ...
                    for r in range(world_size):
                        p = os.path.join(eval_dir, f".class_ids.rank{r}.tmp")
                        if not os.path.exists(p):
                            continue
                        with open(p, "r") as f:
                            merged.extend(line.rstrip("\n") for line in f if line.strip())
                    merged.sort(key=lambda s: int(s.split()[0]))
                    out_path = os.path.join(eval_dir, "class_ids.txt")
                    ...
                    for r in range(world_size):
                        ...
                        if os.path.exists(p):
                            os.remove(p)
                    logger.info(f"    wrote {len(merged)} entries to {out_path}")
